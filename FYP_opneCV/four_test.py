...
while True:
    ...
    if cx_left is not None and cy_left is not None and cx_right is not None and cy_right is not None:
        ...
        # Debug: Print crosshair coordinates
        print(f"Crosshair coordinates: ({screen_eye_x}, {screen_eye_y})")

        # Draw the crosshair at the eye position
        screen.blit(crosshair, (screen_eye_x - crosshair_size // 2, screen_eye_y - crosshair_size // 2))

        # Check if the crosshair is over a piece
        for i, (x, y) in enumerate(piece_positions):
            if correctly_placed[i]:
                continue  # Skip already placed pieces
            rect = pygame.Rect(x, y, piece_width, piece_height)
            if rect.collidepoint(screen_eye_x, screen_eye_y):
                if selected_piece is None:
                    selected_piece = i
                    selection_start_time = time.time()
                elif selected_piece == i:
                    if time.time() - selection_start_time >= 7:
                        # Select the piece after 3 seconds
                        break
            else:
                if selected_piece == i:
                    selected_piece = None
        ...
        # If eye detection is None and a puzzle piece is being dragged around, snap it to its nearest correct position
